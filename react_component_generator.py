...
export function PageShell({ title, subtitle, action, children }: PageShellProps) {
  return (
    ...
    def _generate_layout(self) -> str:
        ...
        dashboard_label = self._t('dashboard')
        ...
        L.append("            </div>")
        L.append("            <div>")
        L.append('              <h1 className="text-sm font-bold tracking-tight text-sidebar-foreground">' + dashboard_label + '</h1>')
        L.append('              <p className="text-xs text-sidebar-foreground/60">' + management_label + '</p>')
        L.append("            </div>")
        L.append("          </div>")
        L.append('          <button')
        ...
        L.append("          </button>")
        L.append('          <span className="font-semibold text-sm">' + dashboard_label + '</span>')
        L.append("        </header>")
  ...
  return (
    ...
    def _render_form_field(self, ctrl_key: str, ctrl_data: dict, fulltype: str, deps: list, unique_deps: list, req_attr: str, select_ph: str) -> list:
        ...
        if fulltype == "string/text":
            ...
            lines.append("  onValueChange={v => setFields(f => ({ ...f, " + ctrl_key + ": v === 'none' ? undefined : v }))}")
            lines.append(">")
